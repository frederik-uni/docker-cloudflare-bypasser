...
@browser(enable_xvfb_virtual_display=True, xvfb_extra_args=["-ac"], window_size=WindowSize.window_size_1920_1080)
...
def click(x, y):
    result = subprocess.run(["python", "clicker.py", str(int(x)), str(int(y))])

    if result.returncode != 0:
        print("Click process failed.")
